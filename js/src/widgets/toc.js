  ...
  $.TableOfContents.prototype = {
    ...
    getTplData: function() {
      ...
      case '1':
        ...
        break;
      case '2':
        ranges = _this.extractV21RangeTrees(_this.structures); //works for 2 and 2.1
        break;
      case '2.1':
        ranges = _this.extractV21RangeTrees(_this.structures); //works for 2 and 2.1
        break;
...
//BH edit
//Fix {{#if canvases}}leaf-item{{else}}has-child{{/if}} instead of
//{{#if children}}has-child{{else}}leaf-item{{/if}}
//Fix 2.0  See if it is aggregating canvases.  If it is, then it is a leaf item.  If it isn't, then it is just an aggregator, regardless if it is empty or not.
    template: function(tplData) {

      var template = Handlebars.compile([
        '<ul class="toc">',
        '{{#nestedRangeLevel ranges}}',
        '<li class="{{#if canvases}}leaf-item{{else}}has-child{{/if}}">',
      ...
      Handlebars.registerHelper('tocLevel', function(id, label, level, children) {
        ...
        //cert = '<i class="fa fa-certificate star"></i>'; //BH edit for styling.  Don't want the the icon caused by fa-certificate.
